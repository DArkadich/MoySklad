#!/usr/bin/env python3
"""
Новая версия обучения ML-моделей с полными данными позиций
"""
...
class PositionsBasedTrainer:
    """Тренер моделей на основе полных данных позиций"""
    ...
    def load_data(self):
        ...
        possible_sales_paths = [
            "/app/data/sales_with_positions.csv",
            "/app/data/sales_history.csv",
            "sales_with_positions.csv",
            "sales_history.csv",
            "/app/sales_with_positions.csv",
    ...
    def extract_sales_by_product(self, sales_df, products):
        """Извлечение продаж по товарам из полных данных позиций"""
        logger.info("Извлечение продаж по товарам из полных данных позиций...")
        
        # Проверяем, есть ли поле positions_data
        if 'positions_data' not in sales_df.columns:
            logger.error("Поле 'positions_data' не найдено в данных продаж")
            logger.info(f"Доступные колонки: {list(sales_df.columns)}")
            return {}
            ...
            if pd.notna(positions_data_str) and positions_data_str:
                try:
                    ...
                    for i, position in enumerate(positions_data):
                        ...
                        # Отладочная информация для первых позиций
                        if processed_rows < 3 and i < 2:
                    ...
                    if processed_rows < 3:
                        logger.info(f"DEBUG: Ошибка в строке {processed_rows + 1}: {e}")
    ...
    def _extract_product_id_from_assortment(self, assortment):
        ...
        try:
            ...
            if href and '/entity/product/' in href:
                ...
                logger.info(f"DEBUG: Извлечен service_id из href: {service_id}")
    ...
    def run(self):
        ...
        logger.info("Запуск обучения ML-моделей на основе полных данных позиций...")
