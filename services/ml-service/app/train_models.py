"""
Скрипт для обучения ML-моделей на данных из МойСклад
"""
...
class ModelTrainer:
    """Класс для обучения моделей на данных МойСклад"""
    ...
    async def train_models(self, training_data: Dict[str, List[Dict]]):
        """Обучение моделей для всех продуктов"""
        ...
        for product_id, features in training_data.items():
            try:
                ...
                if result.get('success', False):
                    trained_models += 1
                    logger.info(f"Модель для продукта {product_id} обучена успешно. Точность: {result.get('accuracy', 0):.3f}")
                else:
                    failed_models += 1
                    logger.error(f"Ошибка обучения модели для продукта {product_id}: {result.get('error', 'Unknown error')}")
...
async def main():
    ...
    try:
        ...
        # Обучаем модели
        trained, failed = await trainer.train_models(training_data)
